...
func (b Build) marshal(declined int) ([]byte, error) {
	if !b.Address.IsValid() {
		return nil, fmt.Errorf("sdpx: build: no advertised address")
	}
	if b.Port <= 0 || b.Port > 65535 {
		return nil, fmt.Errorf("sdpx: build: bad port %d", b.Port)
	}
	if len(b.Codecs) == 0 {
		return nil, ErrNoCommonCodec
	}
	if b.DTLS && (b.Fingerprint == nil || b.ICEUfrag == "" || b.ICEPwd == "") {
		return nil, fmt.Errorf("sdpx: build: DTLS leg needs a fingerprint and ICE credentials")
	}
	...
	conn := &sdp.ConnectionInformation{
		...
		Address:     &sdp.Address{Address: b.Address.String()},
	}
	...
	md := &sdp.MediaDescription{
		MediaName: sdp.MediaName{
			Media:  "audio",
			Port:   sdp.RangedPort{Value: b.Port},
			Protos: b.protos(),
		},
	}
...
func attr(k, v string) sdp.Attribute { return sdp.Attribute{Key: k, Value: v} }

// declinedSection is an RFC 3264 §6 rejected stream: port 0, no
// attributes, no connection information.
func declinedSection() *sdp.MediaDescription {
