...
- **P2-CFG-010 — Dead "bind required" validation branches.** `internal/config/validate_proxy.go:207-209`, `:235-236` (defaults always fill the bind, `proxy.go:404-424`). INFERENCE. Action: delete.
  - Resolution (#41): deleted. Dead code has no observable behaviour, so no test can fail before the deletion. `TestProxyDefaultsAlwaysFillBinds` (`// audit: P2-CFG-010`) instead pins the invariant the deletion relies on: every enabled public listener and the private socket leave `listenerDefaults` with a bind.
- **P2-CFG-011 — Watch misses Kubernetes ConfigMap symlink swaps.** `internal/config/reload.go:61-63`. INFERENCE. Action: fix or document.
  - Resolution (#41): FACT, then fixed. `TestWatchFollowsConfigMapSymlinkSwap` and `TestWatchFollowsSymlinkTargetInOtherDir` (`// audit: P2-CFG-011`) failed on main and pass after `linkTracker`.
