...
- CI (`.github/workflows/ci.yml`, on push to main and PRs): gofmt, `go mod tidy -diff`, vet, build, `check` on both example configs, `go test -race ./...`, and govulncheck (pinned v1.8.0, `GOTOOLCHAIN=local`). `soak.yml` runs nightly and on demand: the trunk soak, the suite under `-race -count=N`, and each `Fuzz*` target for `-fuzztime`. A new fuzz target must be added to its matrix.
