...
func TestMalformedOffersRejected(t *testing.T) {
	...
	// An SDP over sdp.MaxSize goes over the WebSocket listener: UDP would
	...
	if len(huge) <= sdp.MaxSize {
		t.Fatalf("test body is %d bytes, not over the %d-byte limit it means to exercise", len(huge), sdp.MaxSize)
	}
...
// An orphan CANCEL — one sipgo could not match to the INVITE's server
// transaction — is matched to the in-flight attempt by Call-ID. Call-ID
// alone is not enough: RFC 3261 §9.1 requires the CANCEL to carry the
// INVITE's From tag too, so one that names a live Call-ID with somebody
// else's tag must be answered 481 and must leave the call ringing.
func TestOrphanCancelWrongFromTagDoesNotCancel(t *testing.T) {
	h := startHarness(t, false)
	phone := newUDPClient(t)
	...
	invite.SetTransport("UDP")
	invite.SetDestination(h.publicUDP)
	// Both CANCELs are built from a snapshot taken BEFORE the INVITE is
	// handed to the transaction layer — see TestCancelPropagatesUpstream.
	good := buildCancelFor(invite)
	bogus := buildCancelFor(invite)
	// A branch of its own, so sipgo does not match this CANCEL to the
	// INVITE server transaction and it reaches the orphan handler, which
	// is the path that does the Call-ID lookup.
	bogus.Via().Params.Add("branch", "z9hG4bK-orphan-wrongtag")
	bogus.From().Params.Add("tag", "not-the-callers-tag")
	...
	if res := phone.do(t, bogus, h.publicUDP); res.StatusCode != 481 {
		t.Fatalf("CANCEL with a wrong From tag: got %d, want 481", res.StatusCode)
	}
	if got := h.fs.waitFor(sip.CANCEL, 1, 300*time.Millisecond); len(got) != 0 {
		t.Fatalf("FreeSWITCH saw %d CANCELs for a wrong-tag CANCEL, want 0", len(got))
	}
	select {
	case code := <-done:
		t.Fatalf("the INVITE was finalised (%d) by a CANCEL that did not match it", code)
	default:
	}

	// The real CANCEL still works: the attempt was left in place, not taken.
	if res := phone.do(t, good, h.publicUDP); res.StatusCode != 200 {
		t.Fatalf("CANCEL: got %d", res.StatusCode)
	}
	if got := h.fs.waitFor(sip.CANCEL, 1, 5*time.Second); len(got) != 1 {
		t.Errorf("FreeSWITCH saw %d CANCELs, want 1", len(got))
	}
	select {
	case <-done:
	case <-time.After(5 * time.Second):
