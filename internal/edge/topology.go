...
func (t *topology) isSelfVia(v *sip.ViaHeader) bool {
	...
	port := v.Port
	if port == 0 {
		port = fsip.DefaultPort(v.Transport)
	}
...
// upstreamEntryFor returns the resolved endpoint for a node name. Every
// name a dial order produces comes from t.upstreamNames, which is exactly
// this map's key set, so an unknown name is not a case callers have to
// handle — it cannot occur.
func (t *topology) upstreamEntryFor(name string) endpoint { return t.upstreams[name] }
...
func (t *topology) upstreamDialOrder(user string, available func(string) bool) []string {
	if len(t.upstreamNames) == 0 {
		return nil
	}
	pool, cooled := orderByAvailability(t.upstreamNames, available)
...
func (t *topology) selectUpstream(user string, available func(string) bool) (string, endpoint, bool) {
...
// orderByAvailability splits names into the ones the cooldown oracle says
// may be used and the ones still cooling, preserving the input order.
//
...
func (s *Server) selectUpstream(user string) (string, endpoint, bool) {
