...
func TestParsePhoneOffer(t *testing.T) {
	...
	if a.WebRTC() {
		t.Error("plain RTP offer classified as webrtc")
	}
	want := []Codec{
...
func TestNegotiate(t *testing.T) {
	...
	// Which codecs an SBC is willing to relay is plane policy and lives in
	// the proxy package; every codec in this offer is one it relays, so the
	// parsed list is the negotiation input here.
	filtered := offer.Audio.Codecs
	if len(filtered) != 4 {
		t.Fatalf("codecs = %v", Describe(filtered))
	}
	...
	got, err := Negotiate(filtered, answer)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 {
...
func TestNegotiateNoCommonCodec(t *testing.T) {
	...
	if _, err := Negotiate(a, b); !errors.Is(err, ErrNoCommonCodec) {
		t.Fatalf("err = %v", err)
	}
}

// An intersection that leaves only DTMF is not a usable call.
func TestNegotiateDTMFOnlyRejected(t *testing.T) {
	a := []Codec{
		{PayloadType: 0, Name: "PCMU", ClockRate: 8000, Channels: 1},
...
func TestNegotiateChannelsMatter(t *testing.T) {
	a := []Codec{{PayloadType: 111, Name: "opus", ClockRate: 48000, Channels: 2}}
...
func TestNegotiateRejectsRenumbering(t *testing.T) {
	...
	if _, err := Negotiate(offer, same); err != nil {
		t.Errorf("matching payload numbers rejected: %v", err)
	}
	renum := []Codec{{PayloadType: 96, Name: "opus", ClockRate: 48000, Channels: 2}}
	if _, err := Negotiate(offer, renum); !errors.Is(err, ErrRenumbered) {
		t.Errorf("renumbered answer accepted: err = %v", err)
	}
}

// A bare "a=rtcp" or "a=rtcp:" has no port to read. Parsing must not index
// past the end of an empty field list.
func TestParseBareRTCPAttribute(t *testing.T) {
	for _, line := range []string{"a=rtcp\r\n", "a=rtcp:\r\n", "a=rtcp: \r\n"} {
		body := "v=0\r\n" +
			"o=- 1 1 IN IP4 198.51.100.9\r\n" +
			"s=-\r\n" +
			"c=IN IP4 198.51.100.9\r\n" +
		...
		if s.Audio.RTCPPort != 0 {
			t.Errorf("%q: RTCPPort = %d, want 0", line, s.Audio.RTCPPort)
		}
	}
}
