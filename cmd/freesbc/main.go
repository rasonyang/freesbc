...
import (
	...
	"github.com/freesbc/freesbc/internal/proxy"
...
func run(cfgPath string) error {
	...
	// trunk.Server.Run). Only the shield (M6) and admin API (M7) still attach
	// here.
...
func adminDeps(store *config.Store, pool *media.Pool, sipServer *trunk.Server, edge *proxy.Server) admin.Deps {
	deps := admin.Deps{
		Ports:   pool.Stats,
		Version: version,
